...
export default function Home() {
  ...
  const fetchData = async () => {
    const redReq = await fetch(
      `/api/getParsedCards?score=${redParseParams.score}&mods=${redParseParams.mods}&bpm=${redParseParams.bpm}&ar=${redParseParams.ar}&map=${redParseParams.map}&hand=${redParseParams.hand}`,
    );
    const redReqJSON = await redReq.json();
    setRedScore(redReqJSON);

    const blueReq = await fetch(
      `/api/getParsedCards?score=${blueParseParams.score}&mods=${blueParseParams.mods}&bpm=${blueParseParams.bpm}&ar=${blueParseParams.ar}&map=${blueParseParams.map}&hand=${blueParseParams.hand}`,
    );
  ...
  return (
    <main className="flex flex-col gap-5 items-center">
      ...
      <div className="grid grid-cols-2 gap-5 w-[60em]">
        ...
        <div className="flex items-center mb-4 columns-2">
          <label>
            <input
              onChange={() =>
                setRedParseParams((prev) => {
                  return {
                    score: prev.score,
                    mods: prev.mods.includes("hd")
                      ? prev.mods.length == 1
                        ? ["nm"]
                        : prev.mods.filter((mod) => mod !== "hd")
                      : prev.mods.includes("nm")
                        ? ["hd"]
                        : [...prev.mods, "hd"],
                    map: prev.map,
                    bpm: prev.bpm,
            ...
            <input
              onChange={() =>
                setRedParseParams((prev) => {
                  return {
                    score: prev.score,
                    mods: prev.mods.includes("hr")
                      ? prev.mods.length == 1
                        ? ["nm"]
                        : prev.mods.filter((mod) => mod !== "hr")
                      : prev.mods.includes("nm")
                        ? ["hr"]
                        : [...prev.mods, "hr"],
                    map: prev.map,
            ...
            <input
              onChange={() =>
                setRedParseParams((prev) => {
                  return {
                    score: prev.score,
                    mods: prev.mods.includes("dt")
                      ? prev.mods.length == 1
                        ? ["nm"]
                        : prev.mods.filter((mod) => mod !== "dt")
                      : prev.mods.includes("nm")
                        ? ["dt"]
                        : [...prev.mods, "dt"],
                    map: prev.map,
        ...
        <button
          className="bg-gray-400 rounded-lg p-2 hover:bg-gray-200 select-none col-span-2"
          onClick={fetchData}
        >
          <b>Calculate Score</b>
        </button>
