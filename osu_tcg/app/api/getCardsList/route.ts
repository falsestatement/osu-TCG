...
export const GET = async () => {
    const updated = card_database.map((card): CardType => {
        return {
            ...
            effect: [{value: parseFloat(`${card.effect}`) ? parseFloat(`${card.effect}`) : 0, operator: card.operation ? card.operation : "add"}],
            effectiveMods: card.effectiveMods.split(',').filter((mod) => !!mod) as string[],
            effectiveMaps: card.effectiveMaps.split(',').filter((map) => !!map) as string[],
            effectiveAttributes: card.effectiveAttributes.split(',').map((attribute) => parseEffectiveAttribute(attribute)).filter((attribute) => !!attribute) as MapAttribute[],
        }
    })
    console.log(updated);
...
*/
const parseEffectiveAttribute = (attributeStr: string): MapAttribute | undefined => {
    const validOperatorList = ['<', '>', '='];

    // invalid syntax check
    // op at start
    if(validOperatorList.includes(attributeStr[0]))
        return undefined;
